...
| 不透明度 | 单一漏斗：`decideOpacity()` 汇总条件 → `applyOpacity()` 写入 `--wp-opacity`（作用于整棵 `.wp-root`）。优先级自上而下：**拖拽中 → 1**；**边界内驻留(≥180ms) → 1**；**忙碌 → `busyOpacity`(默认 0.25)**；**闲置(指针 3s 未动) → `idleOpacity`(默认 0.6，0.2–1.0)**；否则 1。变淡 `.4s`、恢复 `.12s`。边界必须压过忙碌 —— 否则工作时鼠标一快鲸鱼就淡下去，根本没法瞄准点击 |
| 全局光标轮询 | 主进程 20Hz（`CURSOR_TICK_MS=50`）`screen.getCursorScreenPoint()` → `cursor:tick` IPC。渲染进程只能拿到窗口内的指针事件（靠 `setIgnoreMouseEvents(forward)` 转发），**窗口外的全局位置拿不到**，而「忙碌判定」必须看窗口外 —— 故由主进程统一轮询，一次轮询同时喂**三个**消费者：眼睛追踪、忙碌判定、悬停边界。窗口不存在/不可见时跳过本拍。**与 §4.2 拖拽光标缓存无关**：§4.2 废弃的是一条把缓存值当作**窗口位置权威**的通道（缓存冻结 → 位置卡住）；此处只把缓存值当**只读的移动量证据**，偏旧或短暂冻结不会造成任何错误状态（冻结 = 速度变 0 = 退出忙碌，而拖拽期间 `decideOpacity()` 本来就恒返回 1），故 §4.2 记录的 Windows 冻结问题在此不适用 |
| 忙碌判定 | `feedCursor()` 维护 2.5s 滑动窗口的**光标平均速度**（非瞬时速度：单帧抖动不影响，人工作时是持续快速移动）。双阈值回滞：`>600` DIP px/s 进入、`<200` 退出，避免在临界速度上反复横跳导致透明度闪烁。轮询中断（窗口隐藏/息屏）超过一个窗口宽即清空样本，否则累积位移会被误判成一次瞬移。实测：进入延迟 ~0.3s，停止移动后 ~2.65s 退出。写 `.wp-busy` 类并由 `decideOpacity()` 取用 |
| 悬停边界 | 鲸鱼图盒向外扩张 `max(24px, 宽×0.25)`（随缩放等比变化），气泡展开时并入气泡盒（否则停在气泡上读字会被连气泡一起淡掉）。**用全局光标判定，不用窗口内 pointermove**：窗口恰好把鲸鱼卡在右下角，右/下**没有余量**，靠窗口事件从右侧靠近根本触发不到；全局判定不受窗口矩形约束、四边余量对称，且是纯几何，不捕获任何点击 → **天然穿透**。`?debugBoundary=1` 可开虚线叠层（生产无开销） |
| 眨眼 | 独立随机定时 3–9s（均值 6s ≈ 10 次/分），与追踪互不干扰、两者叠加。闭眼帧 `assets/DSniang1-closed.png` 是**离线生成**的同尺寸图：把两只眼盘整块填成面部主肤色、保留上方那条深色睫毛带，底部残留的一道浅弧正好当闭眼睑褶。渲染上是叠一层 `.wp-img-lid` 做 opacity 切换（`.06s`），闭合 95ms。**只在内置默认主图下启用**（`eyeFxEnabled()`：`mainImgPath === 'assets/DSniang1.png'` 且当前未在显示预警图）—— 自定义图没有配套闭眼帧，硬套会把 DSniang1 的闭眼帧叠到用户的图上 |
| 朝光标倾斜 | 20Hz 光标位置 → `updatePose()` 写 `--wp-tilt-{x,y,r}`，`.wp-img` 据此 `translate + rotate`，轴在底边中点。位移以 `--wp-u` 为单位随缩放联动，满偏 = 鲸鱼半宽 ×2.5（约 ±14u / ±2.2°），超出即归零（光标跑到屏幕另一头时鲸鱼不该还歪着）；镜子朝向（`.wp-left`）时 x 分量取反。过渡 `.18s` 把 20Hz 的离散采样抹成连续运动。几何基准取 `offsetLeft/offsetWidth`（布局盒，**不含 transform**）—— 用 `getBoundingClientRect()` 会把倾斜本身算进去，20Hz 下形成反馈环而低频抖动 |
| 瞳孔跟随 | 与倾斜共用 `updatePose()` 的同一套几何量（`--wp-gaze-{x,y}`），但饱和更快：1.2 倍半宽即到边（约 ±12u 横向 / 上 15u / 下 3u）。**上下刻意不对称**：虹膜上移会贴上睫毛（上方本来就没有空隙）、只在下方留一道月牙，读作眼窝阴影；下移则会在虹膜和睫毛之间裂开一条缝，像眼球掉下来了。鲸鱼贴在屏幕右下角，光标绝大多数时候在它上方，走的正好是空间充裕的那一侧 |
| 眼睛追踪的三层合成 | **不裁剪、不遮罩、不扩展虹膜**，靠普通 alpha 叠加解决「虹膜铺满整只眼、移动会露出皮肤」的问题。自下而上：① `assets/DSniang1-gaze-iris.png` —— 整张透明，只有两个眼盘，外加一圈向外逐渐变暗的延伸（BFS 距离场生成，颜色从该处虹膜边缘色插值到描边色 `#3D4774`），**会平移**；② `assets/DSniang1-gaze-base.png` —— 原图把眼盘内部挖成透明，其余像素原样，**静止不动**；③ 闭眼帧（眨眼用）。虹膜平移时前缘滑进底图的不透明区被整块挡掉、后缘让出的空档露出自己的暗色延伸，读作眼窝内阴影。于是**可动范围不受眼眶粗细限制**，也不必知道眼型（右眼那条刘海完全无关，只是底图的一部分）。静止态实测与原图**逐像素相同**：挖孔只吃掉严格判据命中的虹膜像素，抗锯齿边缘留在底图里盖在虹膜层边缘上 |
| 眼睛追踪的启用条件 | 与眨眼同一条件（`eyeFxEnabled()`）：内置默认主图且当前未显示预警图。关闭时整层 `display:none`（`.wp-root:not(.wp-eye-fx)`），主图换回原图 —— 自定义图未必处处不透明，光靠「主图会盖住它」太脆 |
