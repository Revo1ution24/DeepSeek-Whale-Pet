...
const DEFAULTS = {
  ...
  busyFade: true,           // 忙碌（鼠标高速移动）时降不透明度（开关）
  busyOpacity: 0.25,        // 忙碌时的不透明度（0.05 - 1.0）
  ...
  autostart: false,
  harnessPath: '',          // DeepSeek-Harness 便携版所在文件夹（托盘「打开 Harness」用）
...
function sanitize(raw) {
  ...
  if (typeof d.busyFade === 'boolean') cfg.busyFade = d.busyFade
  if (d.busyOpacity !== undefined) cfg.busyOpacity = Math.round(num(d.busyOpacity, 0.05, 1, DEFAULTS.busyOpacity) * 100) / 100
  ...
  if (typeof d.harnessPath === 'string') cfg.harnessPath = d.harnessPath.trim()
